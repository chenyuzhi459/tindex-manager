...
$(document).ready(function() {
  var augment = function(data) {
    for (i = 0 ; i < data.length ; i++) {
      ...
      data[i].more =
        '<a target="_blank" href="/druid/indexer/v1/task/' + taskId + '">payload</a>' +
        '<a target="_blank" href="/druid/indexer/v1/task/' + taskId + '/status">status</a>' +
        '<a target="_blank" href="/druid/indexer/v1/task/' + taskId + '/log">log (all)</a>' +
        '<a target="_blank" href="/druid/indexer/v1/task/' + taskId + '/log?offset=-8192">log (last 8kb)</a>'
    }
  }
